...
static std::unique_ptr<GLTextureSurfaceFactory> surface_factory_;
...
#include <memory>
static std::shared_ptr<MyApp> app;

void UltralightHelper::SetupContext()
{
	app = std::make_shared<MyApp>();

}

void UltralightHelper::Update()
{
	app->Update();
}

void UltralightHelper::Render()
{
	app->Render();
}
