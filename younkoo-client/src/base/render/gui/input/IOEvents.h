...
namespace YounkooIO {
	...
	struct IOEvent {
		EventType type;
		IOEvent(EventType type) : type(type) {}
		virtual ~IOEvent() {}
	};
	...
	{
		MousePosEvent(double posX = 0, double posY = 0) noexcept
	...
	{
		MouseEvent(int button = 0, int action = 0) noexcept
	...
	{
		WheelEvent(double offsetX = 0, double offsetY = 0) noexcept :
	...
	{
		CharEvent(unsigned int codepoint = 0) noexcept
